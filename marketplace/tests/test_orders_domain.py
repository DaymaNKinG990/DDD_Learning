...
class TestOrderItem:
    ...
    def test_update_order_item_quantity(self):
        ...
        updated_item = item.update_quantity(3)
        
        assert updated_item.quantity == 3
        assert updated_item.total_price == Decimal("150")  # 3 * 50
...
class TestOrder:
    ...
    def test_add_item_to_order(self):
        ...
        assert updated_order.items[0].product_id == "prod_123"
        assert updated_order.total.subtotal == Decimal("100")
        assert updated_order.total.total > Decimal("0")
    ...
    def test_add_duplicate_item_to_order(self):
        ...
        order = order.add_item(item1)
        updated_order = order.add_item(item2)
        
        assert len(updated_order.items) == 1
        assert updated_order.items[0].quantity == 3  # 2 + 1
        assert updated_order.items[0].total_price == Decimal("150")  # 3 * 50
    ...
    def test_remove_item_from_order(self):
        ...
        updated_order = order.remove_item("prod_123")
    ...
    def test_ship_order(self):
        ...
        assert shipped_order.id == order.id
    ...
    def test_ship_pending_order(self):
        ...
        with pytest.raises(ValueError, match="Only confirmed or processing orders can be shipped"):
            order.ship()
    ...
    def test_cancel_delivered_order(self):
        ...
        order = order.confirm().ship().deliver()
        
        with pytest.raises(ValueError, match="Cannot cancel delivered or refunded order"):
            order.cancel()
    ...
    def test_refund_order(self):
        ...
        assert refunded_order.id == order.id