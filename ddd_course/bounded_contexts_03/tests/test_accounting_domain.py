...
class TestInvoice:
    ...
    def test_cancel_paid_invoice_fails(self):
        ...
        # Проверка
        with pytest.raises(
            ValueError, match="Невозможно аннулировать счет в текущем статусе"
        ):
            invoice.cancel()
...
class TestPayment:
    ...
    def test_create_payment(self):
        ...
        # Действие
        payment = Payment(
            invoice_id=invoice_id, amount=amount, payment_method=payment_method
        )
...
class TestFinancialPeriod:
    ...
    def test_create_period(self):
        ...
        # Действие
        period = FinancialPeriod(name=name, start_date=start_date, end_date=end_date)
    ...
    def test_archive_open_period_fails(self):
        ...
        # Проверка
        with pytest.raises(
            ValueError, match="Невозможно архивировать незакрытый период"
        ):
            period.archive()
    ...
    def _create_test_period(self) -> FinancialPeriod:
        ...
        return FinancialPeriod(
            name="Июнь 2023", start_date=date(2023, 6, 1), end_date=date(2023, 6, 30)
        )
...
class TestAccountingService:
    ...
    def test_create_invoice(self):
        ...
        invoice = service.create_invoice(
            guest_id=guest_id, items=items, due_date=due_date
        )
