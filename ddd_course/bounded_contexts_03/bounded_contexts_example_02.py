...
class ProductRepository:
    ...
    def find_by_category(self, category: str) -> List[Product]:
        return [
            p for p in self._products.values() if p.category == category and p.is_active
        ]
...
class PaymentService:
    ...
    def create_payment(self, order: Order) -> Payment:
        ...
        payment = Payment(id=uuid4(), order_id=order.id, amount=order.total_amount)
...
def demonstrate_bounded_contexts():
    ...
    print(
        f"Создан платеж {payment.id} на сумму "
        f"{payment.amount.amount} {payment.amount.currency}"
    )
    ...
    for item in order.items:
        print(
            ...
            f"{item.unit_price.amount} {item.unit_price.currency}"
        )
