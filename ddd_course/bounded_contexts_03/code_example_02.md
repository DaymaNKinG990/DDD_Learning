# Пример кода: Ограниченные контексты

Этот пример демонстрирует взаимодействие нескольких Ограниченных Контекстов в рамках системы электронной коммерции: `Каталог товаров`, `Заказы` и `Платежи`.

- **Shared Kernel (Общее ядро):** Типы `Money` и `OrderStatus` используются несколькими контекстами.
- **Изоляция:** Каждый контекст имеет свои собственные модели и репозитории (`Product`, `Order`, `Payment`). Например, в контексте `Заказы` есть `OrderItem`, который содержит только необходимые данные о товаре (`product_id`, `product_name`, `unit_price`), а не всю сущность `Product` из контекста `Каталог`.
- **Интеграция:** В конце показан простой сценарий, как эти контексты могут взаимодействовать для обработки заказа клиента.
