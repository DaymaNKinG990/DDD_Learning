...
class AccountingService:
    ...
    def create_invoice(
        ...
        invoice.number = (
            f"INV-{datetime.utcnow().strftime('%Y%m%d')}-{str(invoice.id)[:8].upper()}"
        )
...
class FinancialReport:
    ...
    def __init__(
        self, period: FinancialPeriod, invoices: List[Invoice], payments: List[Payment]
    ):
        self.period = period
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "payment_count": len(
                    [p for p in self.payments if p.status == PaymentStatus.COMPLETED]
                ),
            },
            "payment_methods": {
