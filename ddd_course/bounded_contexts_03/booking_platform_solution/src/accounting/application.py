...
from .domain import (
    ...
    InvoiceItem,
    FinancialReport,
    AccountingService as DomainAccountingService
)
...
class AccountingApplicationService(IAccountingService):
            ...
            if hasattr(self, 'email_service'):
                await self.email_service.send_invoice(
                    ...
                    context={
                        ...
                        "total_amount": f"{invoice.total.amount:.2f} {invoice.currency}"
                    }
                )
    ...
    async def record_payment(
        ...
        try:
            ...
            if process_online:
                try:
                    ...
                    if result.get("status") == "completed":
                        ...
                        payment.fail(reason=result.get("error_message", "Неизвестная ошибка"))
                except Exception as e:
            ...
            if payment.status == PaymentStatus.COMPLETED:
                await self.domain_service.apply_payment(invoice, payment)
                await self.uow.invoices.save(invoice)
            
            await self.uow.commit()
            
            # Отправляем уведомление (если настроено)
            if hasattr(self, 'email_service') and payment.status == PaymentStatus.COMPLETED:
                await self.email_service.send_payment_confirmation(
                    ...
                    context={
                        "invoice": invoice,
                        "payment": payment,
                        "payment_date": payment.processed_at.strftime("%d.%m.%Y %H:%M") if payment.processed_at else "",
                        "amount": f"{payment.amount.amount:.2f} {payment.amount.currency}"
                    }
                )
        ...
        try:
            ...
            if process_online and original_payment.transaction_id:
                try:
                    ...
                    if result.get("status") != "completed":
                        refund.fail(reason=result.get("error_message", "Неизвестная ошибка"))
            ...
            # Обновляем статус счета
            await self.domain_service.apply_payment(invoice, refund)
            await self.uow.invoices.save(invoice)
    ...
    async def generate_financial_report(
        ...
        # В реальном приложении здесь была бы логика генерации отчета
        # с использованием report_generator
        
        # Получаем все счета и платежи за период
        invoices = []
        payments = []
        
        # В реальном приложении использовались бы методы репозиториев
        # для получения данных за указанный период
        
        # Формируем отчет
        report = FinancialReport(
            period=FinancialPeriod(
                id=uuid4(),
                name=f"Отчет за период с {start_date} по {end_date}",
        ...
        # Преобразуем отчет в нужный формат
