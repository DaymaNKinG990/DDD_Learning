...
class InMemoryPaymentRepository(IPaymentRepository):
    ...
    async def list_by_guest(
        ...
        for pay_id in sorted(payment_ids, reverse=True):
            ...
            if start_date and payment.processed_at and payment.processed_at.date() < start_date:
                continue
                
            if end_date and payment.processed_at and payment.processed_at.date() > end_date:
                continue
    ...
    async def list_by_status(
        ...
        for payment in sorted(self._payments.values(), key=lambda x: x.created_at, reverse=True):
    ...
    async def save(self, payment: Payment) -> None:
        ...
        # Примечание: для индексации по гостю нужен дополнительный запрос к репозиторию счетов
...
class InMemoryFinancialPeriodRepository(IFinancialPeriodRepository):
    ...
    async def get_current_period(self) -> Optional[FinancialPeriod]:
        ...
        today = date.today()
        for period in self._periods.values():
            if period.status == FinancialPeriodStatus.OPEN and period.start_date <= today <= period.end_date:
                return period
        return None
...
class ConsoleEmailService(IEmailService):
    ...
    async def send_invoice(
        ...
        print(f"[EMAIL] Отправка счета {invoice.number} на {to_email}")
        print(f"Шаблон: {template_name}")
        print(f"Контекст: {json.dumps(context or {}, default=str, ensure_ascii=False, indent=2)}")
        return True
    ...
    async def send_payment_confirmation(
        ...
        print(f"[EMAIL] Отправка подтверждения платежа {payment.id} на {to_email}")
        print(f"Шаблон: {template_name}")
        print(f"Контекст: {json.dumps(context or {}, default=str, ensure_ascii=False, indent=2)}")
        return True
