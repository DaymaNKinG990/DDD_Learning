...
from shared_kernel import (
    EntityId, Money, DateRange, DomainEvent, DomainException,
    BusinessRuleValidationException, BookingStatus, RoomType, generate_id
)
...
class BookingPolicy:
    ...
    def validate_booking_period(cls, period: DateRange) -> None:
        ...
        if period.check_in <= today:
            raise BusinessRuleValidationException(
                "Дата заезда должна быть в будущем"
            )
