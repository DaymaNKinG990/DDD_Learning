# Решение упражнения: Единый язык

Это пример решения для упражнения по разработке системы управления задачами.

Ключевые моменты реализации:

- **Явные имена:** Классы `Task`, `Sprint`, `TeamMember` и перечисления `TaskStatus`, `TaskPriority` точно соответствуют понятиям из предметной области.
- **Объекты-значения:** `TaskDescription` и `Comment` инкапсулируют простые, но важные концепции, добавляя им валидацию и смысл.
- **Поведение в сущностях:** Методы `assign`, `update_status`, `add_comment` находятся внутри сущности `Task`, так как они оперируют её состоянием и обеспечивают инварианты.
- **Доменные сервисы:** `SprintPlanningService` инкапсулирует логику, которая не принадлежит ни одной конкретной сущности (завершение спринта затрагивает и сам спринт, и множество задач).
...
class TaskRepository:
    ...
    def find_by_sprint_and_status_not(
        ...
        """Найти задачи в спринте, исключая определённый статус."""
        raise NotImplementedError
