...
@pytest.fixture
def task(product_owner: TeamMember, active_sprint: Sprint, task_description: TaskDescription) -> Task:
...
def test_task_assign(task: Task, developer: TeamMember, product_owner: TeamMember):
    ...
    task.assign(developer.id, product_owner.id)  # Повторное назначение тому же исполнителю
    assert task.updated_at == old_updated_at  # Время не должно измениться


def test_task_status_transitions(task: Task, developer: TeamMember, product_owner: TeamMember):
...
def test_task_workflow(product_owner: TeamMember, developer: TeamMember, active_sprint: Sprint):
    """Проверяет полный рабочий процесс по задаче."""
    # Создаём описание задачи
    description = TaskDescription(
        summary="Реализовать корзину покупок",
        details="Необходимо реализовать функционал корзины с добавлением/удалением товаров"
    )
    ...
    task.add_comment("Убедитесь, что корзина сохраняется после перезагрузки страницы", product_owner.id)
    ...
    assert len(task.comments) >= 5  # Как минимум 5 комментариев (назначение, метки, смена статусов)
