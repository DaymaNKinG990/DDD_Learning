...
class Task:
    ...
    def assign(self, assignee_id: UUID, current_user_id: UUID) -> None:
        ...
        self._add_system_comment(
            f"Задача назначена на пользователя {assignee_id}", current_user_id
        )
    ...
    def add_comment(self, content: str, author_id: UUID) -> Comment:
        ...
        """
        comment = Comment(id=uuid4(), author_id=author_id, content=content)
...
class SprintPlanningService:
    ...
    def complete_sprint(
        self, sprint_id: UUID, new_sprint: "Sprint", user_id: UUID
    ) -> None:
        ...
        incomplete_tasks = self.task_repository.find_by_sprint_and_status_not(
            sprint_id=sprint_id, status=TaskStatus.DONE
        )
        ...
        for task in incomplete_tasks:
            ...
            task.add_comment(
                f"Перенесено из спринта '{current_sprint.name}' "
                f"в спринт '{new_sprint.name}'",
                user_id,
            )
...
class TaskRepository:
    ...
    def find_by_sprint_and_status_not(
        self, sprint_id: UUID, status: TaskStatus
    ) -> List[Task]:
...
def demonstrate_task_management():
    ...
    task.add_comment(
        "Убедитесь, что поддерживается двухфакторная аутентификация", product_owner.id
    )
    ...
    # Создаём доску задач
    board = Board(id=uuid4(), name="Доска проекта", project_id=uuid4())
