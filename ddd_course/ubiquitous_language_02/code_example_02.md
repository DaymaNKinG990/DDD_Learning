# Пример кода: Единый язык

В этом примере мы рассмотрим, как принципы Единого языка (Ubiquitous Language) применяются при создании системы бронирования отелей. Обратите внимание, как названия классов (`Booking`, `Guest`, `Room`), методов (`cancel`, `check_in`) и атрибутов (`stay_period`, `total_price`) напрямую отражают бизнес-термины.

Это делает код не только более читаемым для разработчиков, но и понятным для бизнес-аналитиков и других участников проекта, которые знакомы с предметной областью.
