...
9.  Какой из следующих классов в Python лучше всего подходит для реализации Объекта-значения?
    *   **A)** Обычный класс с изменяемыми атрибутами.
    *   **B)** `dataclass` с параметром `frozen=True`.
    *   **C)** `namedtuple` из модуля `collections`.
    *   **D)** Любой из перечисленных вариантов.

10. Что из перечисленного является преимуществом использования Объектов-значений?
    *   **A)** Упрощение работы с базой данных.
    *   **B)** Повышение выразительности доменной модели и инкапсуляция логики, связанной с определенными значениями.
